...
export default function AdminOrdersQueue() {
  ...
  // 2. Fetch Invoices to link completed orders to their PDF previews
  ...
  // Map order_id to invoice details (id and invoice_number)
  ...
  // Map order_id to purchase orders list
  const poMap = useMemo(() => {
  ...
  // Compute Pending Orders (newest first)
  ...
  // Compute All Orders (newest first)
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-150">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Review active restaurant requests and inspect full order logs.</p>
        </div>
        <Link href="/orders/upload" passHref>
          <Button className="bg-green-700 hover:bg-green-800 font-semibold shadow-sm">
      ...
      <div className="flex space-x-6 border-b border-gray-200">
        ...
        >
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${activeTab === "all" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}>
            {allOrders.length}
          </span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {displayedOrders.length === 0 && (
          <div className="py-16 text-center text-gray-500 bg-white rounded-lg border border-dashed border-gray-200">
            {activeTab === "pending" ? (
              <p className="font-semibold text-gray-600">No pending orders! You are all caught up.</p>
            ) : (
              <p className="font-semibold text-gray-600">No order logs found.</p>
            )}
        ...
        {displayedOrders.map(order => {
          ...
          return (
            <Card key={order.id} className={`flex flex-col border shadow-sm transition-all duration-200 ${!isPending ? "bg-gray-50/30 border-gray-200" : "bg-white border-gray-200 hover:border-green-500"}`}>
              ...
              >
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-3 flex-wrap">
                    <h3 className="font-bold text-gray-900 text-lg leading-tight truncate">{order.customer?.restaurant_name || "Unknown Customer"}</h3>
                  ...
                  <div className="flex gap-4 text-xs text-gray-500 mt-1">
                    ...
                    <span>•</span>
                    <span><strong>{order.items.length}</strong> items requested</span>
                  </div>
                </div>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-gray-100 pt-4 bg-gray-50/20">
                  {/* Grouped Items List */}
                  <div className="mb-4 space-y-3">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Requested Items List</h4>
                    {Object.entries(getGroupedItems(order.items)).map(([groupName, groupItems]) => (
                      <div key={groupName} className="space-y-1">
                        <div className="text-[11px] font-bold text-gray-400 pl-1 uppercase tracking-wide">
                          {groupName} ({groupItems.length})
                        </div>
                        <div className="border border-gray-200 rounded-md overflow-hidden bg-white divide-y divide-gray-150 shadow-sm">
                          {groupItems.map((item: any) => (
                            <div key={item.id} className="p-3 flex justify-between items-center text-sm">
                              ...
                              <span className="font-bold text-green-700 bg-green-50/50 px-2.5 py-0.5 rounded border border-green-100 text-xs">
                  ...
                  {order.remarks && (
                    <div className="text-xs bg-gray-50 p-2.5 rounded border border-gray-100 text-gray-600 mb-4">
                  ...
                  <div className="mb-6 pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Supplier Purchase Orders</h4>
                      {(!poMap.get(order.id) || poMap.get(order.id)?.length === 0) && ["PENDING", "SUBMITTED", "DRAFT", "REVIEWED"].includes(order.status.toUpperCase()) && (
                        <Button 
                          ...
                          className="h-7 text-xs bg-green-700 hover:bg-green-800 text-white font-medium"
                        >
                    ...
                    {(!poMap.get(order.id) || poMap.get(order.id)?.length === 0) ? (
                      <div className="text-xs bg-gray-50/50 text-gray-400 italic p-3 rounded border border-dashed border-gray-200">
                      ...
                      <div className="space-y-2">
                        {poMap.get(order.id)?.map((po) => (
                          <div key={po.id} className="p-3 bg-white border border-gray-200 rounded-md shadow-xs flex items-center justify-between text-xs">
                            <div className="space-y-0.5">
                              <span className="font-semibold text-gray-800">{po.supplier?.name || "Unassigned Supplier"}</span>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button 
                                ...
                                className="h-7 text-[10px] flex items-center gap-1 border-green-200 text-green-700 hover:bg-green-50"
                              >
                                {copiedPoId === po.id ? <Check className="w-3 h-3" /> : <Send className="w-3 h-3" />}
                                {copiedPoId === po.id ? "Copied!" : "WhatsApp"}
                              </Button>
                              <Link href="/purchase-orders">
                                <Button size="sm" variant="ghost" className="h-7 px-2 text-[10px] text-gray-500">
                                  Receive
                                </Button>
                              </Link>
                            </div>
                  ...
                  {/* Actions Container */}
                  <div className="flex flex-col sm:flex-row gap-2.5 justify-end border-t border-gray-150 pt-4">
                    {!invDetails ? (
                      <div className="flex gap-2 w-full sm:w-auto">
                        {(!poMap.get(order.id) || poMap.get(order.id)?.length === 0) && ["PENDING", "SUBMITTED", "DRAFT", "REVIEWED"].includes(order.status.toUpperCase()) && (
                          <Button 
                            ...
                            disabled={isGeneratingPOsId === order.id}
                            variant="outline"
                            className="w-full sm:w-auto border-green-200 text-green-700 hover:bg-green-50 font-semibold"
                          >
                            Split Suppliers
                          </Button>
                        )}
                        <Link href={`/orders/${order.id}/billing`} passHref className="flex-1 sm:flex-initial">
                          <Button className="w-full bg-green-700 hover:bg-green-800 font-semibold shadow-sm">
                            Generate Invoice
                          </Button>
                        </Link>
                      </div>
                      ...
                      <div className="flex gap-2 w-full sm:w-auto">
                        <Link href={`/invoices/${invDetails.id}`} passHref className="flex-1 sm:flex-initial">
                          <Button variant="outline" className="w-full border-blue-200 hover:bg-blue-50 text-blue-700 font-semibold shadow-sm flex items-center justify-center gap-1.5">
                        ...
                        <Button 
                          ...
                          className="flex-1 sm:flex-initial bg-blue-700 hover:bg-blue-800 text-white font-semibold shadow-sm flex items-center justify-center gap-1.5"
                        >
