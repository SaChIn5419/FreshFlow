...
export default function PackingPage() {
  ...
  const handleToggleItemPacked = async (item: PackingListItem) => {
    ...
    try {
      ...
      const updatedItem = await packingService.updatePackingItem(item.id, {
        is_packed: newStatus,
        quantity_packed: newStatus ? item.quantity_requested : 0,
      });

      // Refresh active packing list locally
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Warehouse Packing Desk</h1>
          <p className="text-sm text-gray-500">Interactive touch-checklist for packing restaurant produce orders.</p>
        </div>
      </div>

      {/* Controls Bar */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
        <div className="flex items-center space-x-2 bg-gray-50 p-1.5 rounded-md border border-gray-200 flex-1 max-w-md">
          <Search className="w-4 h-4 text-gray-400 ml-2" />
          <Input
            placeholder="Search restaurant orders to pack..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="border-0 focus-visible:ring-0 focus-visible:ring-offset-0 px-2 bg-transparent text-sm"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className={filterStatus === "packed" ? "bg-green-700 hover:bg-green-800 text-white" : ""}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          <div className="col-span-full p-12 text-center text-gray-500 bg-white border border-gray-200 rounded-lg">
          ...
          displayedOrders.map((order) => {
            ...
            return (
              <Card 
                key={order.id} 
                className={`hover:border-green-600 transition-all duration-200 cursor-pointer ${isPacked ? "bg-green-50/20 border-green-200" : "bg-white"}`}
                onClick={() => handleOpenChecklist(order.id)}
              >
                <CardContent className="p-5 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg">{order.customer?.restaurant_name || "Restaurant"}</h3>
                      <p className="text-xs text-gray-500 font-mono mt-0.5">Order #{order.id.slice(0, 8)}</p>
                    </div>
                    <Badge variant={isPacked ? "success" : isPartial ? "warning" : "secondary"}>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-xs font-semibold">
                      ...
                      <span className={isPacked ? "text-green-700 font-bold" : "text-amber-700 font-bold"}>
            ...
            <>
              <DialogHeader>
                <div className="flex items-center justify-between pr-6">
                  ...
                  <Badge variant={activePackingList.status === "Packed" ? "success" : "warning"} className="text-sm px-3 py-1">
              ...
              <div className="space-y-4 py-3">
                ...
                <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                  ...
                  />
                  <Button size="sm" variant="ghost" onClick={handleSavePackerName} className="h-8 text-xs">
                    Save
                  </Button>
                </div>

                {/* Progress Card */}
                <div className="bg-green-50/50 p-4 rounded-lg border border-green-100 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold text-green-800 uppercase tracking-wider">Packed Status</div>
                    <div className="text-sm font-semibold text-green-950 mt-0.5">
                  ...
                  <Button 
                    ...
                    className="bg-green-700 hover:bg-green-800 text-white font-semibold flex items-center gap-1.5"
                  >
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Items Checklist</h4>
                  <div className="divide-y divide-gray-150 border border-gray-200 rounded-lg overflow-hidden bg-white shadow-xs">
                    {activePackingList.items.map((item) => (
                      <div
                        ...
                        className={`p-4 flex items-center justify-between cursor-pointer select-none transition-all duration-150 ${
                          item.is_packed ? "bg-green-50/30" : "hover:bg-gray-50"
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className={`font-semibold text-base ${item.is_packed ? "line-through text-gray-500" : "text-gray-900"}`}>
                        ...
                        <div className="text-right">
                          <span className={`text-base font-bold px-3 py-1 rounded-md ${item.is_packed ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}>
